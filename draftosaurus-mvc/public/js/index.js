...
function validateEquality(dropZone, newDinoType) {
    ...
    if (existingDinos.length >= 6) {
        alert(t('validation.equality.limit') || 'En el área de Igualdad no se pueden colocar más de 6 dinosaurios');
        return false;
    }
    ...
    if (existingDinos.length > 0) {
        ...
        if (firstDinoType !== newDinoType) {
            alert(t('validation.equality.mismatch') || 'En el área de Igualdad solo se pueden colocar dinosaurios del mismo tipo');
            return false;
        }
    }
...
function validateThree(dropZone) {
    ...
    if (existingDinos.length >= 3) {
        alert(t('validation.three.limit') || 'En el área de Tres no se pueden colocar más de 3 dinosaurios');
        return false;
    }
...
function validateLove(dropZone) {
    ...
    if (existingDinos.length >= 6) {
        alert(t('validation.love.limit') || 'En el área del Amor no se pueden colocar más de 6 dinosaurios');
        return false;
    }
...
function validateKing(dropZone) {
    ...
    if (existingDinos.length >= 1) {
        alert(t('validation.king.limit') || 'En el área del Rey solo se puede colocar 1 dinosaurio');
        return false;
    }
...
function validateDiversity(dropZone, newDinoType) {
    ...
    if (existingDinos.length >= 6) {
        alert(t('validation.diversity.limit') || 'En el área de la Diversidad no se pueden colocar más de 6 dinosaurios');
        return false;
    }
    ...
    for (let dino of existingDinos) {
        ...
        if (existingType === newDinoType) {
            alert(t('validation.diversity.duplicate') || 'En el área de la Diversidad no se pueden colocar dinosaurios del mismo tipo');
            return false;
        }
    }
...
function validateOne(dropZone) {
    ...
    if (existingDinos.length >= 1) {
        alert(t('validation.one.limit') || 'En el área de Uno solo se puede colocar 1 dinosaurio');
        return false;
    }
...
function validateDrop(dropZone, draggedElement) {
    ...
    if (!dinoType) {
        alert(t('validation.error.unknownType') || 'Error: No se pudo identificar el tipo de dinosaurio');
        return false;
    }
    ...
    if (isFromPanel) {
        ...
        if (currentTotal >= 12) {
            alert(t('validation.limit.total12') || 'No se pueden colocar más de 12 dinosaurios en el parque');
            return false;
        }
    }
...
function calculateKingPoints() {
    ...
    const question = t('king.prompt', { count: totalDinosOfType, dino: dinoNameSpanish }) || `Tienes ${totalDinosOfType} ${dinoNameSpanish}. ¿Otros jugadores tienen menos? Y/N`;
...
function calculateTotalScore() {
    ...
    console.log(t('score.console.detailed') || 'Puntuación detallada:');
    console.log(`${t('score.label.equality') || 'Igualdad'}: ${equalityPoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.three') || 'Tres'}: ${threePoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.love') || 'Amor'}: ${lovePoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.king') || 'Rey'}: ${kingPoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.diversity') || 'Diversidad'}: ${diversityPoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.one') || 'Uno'}: ${onePoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.river') || 'Río'}: ${riverPoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.trexBonus') || 'Bonus T-Rex'}: ${trexBonusPoints} ${t('score.units.points') || 'puntos'}`);
    console.log(`${t('score.label.total') || 'TOTAL'}: ${totalScore} ${t('score.units.points') || 'puntos'}`);
...
function finishGame() {
    ...
    if (totalDinosOnField !== 12) {
        alert(t('validation.need12', { count: totalDinosOnField }) || `¡Atención! Necesitas colocar exactamente 12 dinosaurios en el parque para finalizar la partida. Actualmente tienes ${totalDinosOnField} dinosaurios.`);
    ...
    const detailedResults = `
        <h2>${t('score.finalTitle') || 'Puntuación Final'}</h2>
        <div style="text-align: left; max-width: 400px; margin: 0 auto;">
            <p><strong>${t('score.label.equality') || 'Igualdad'}:</strong> ${equalityPoints} ${t('score.units.points') || 'puntos'}</p>
            <p><strong>${t('score.label.three') || 'Tres'}:</strong> ${threePoints} ${t('score.units.points') || 'puntos'}</p>
            <p><strong>${t('score.label.love') || 'Amor'}:</strong> ${lovePoints} ${t('score.units.points') || 'puntos'}</p>
            <p><strong>${t('score.label.king') || 'Rey'}:</strong> ${kingPoints} ${t('score.units.points') || 'puntos'}</p>
            <p><strong>${t('score.label.diversity') || 'Diversidad'}:</strong> ${diversityPoints} ${t('score.units.points') || 'puntos'}</p>
            <p><strong>${t('score.label.one') || 'Uno'}:</strong> ${onePoints} ${t('score.units.points') || 'puntos'}</p>
            <p><strong>${t('score.label.river') || 'Río'}:</strong> ${riverPoints} ${t('score.units.points') || 'puntos'}</p>
            <p><strong>${t('score.label.trexBonus') || 'Bonus T-Rex'}:</strong> ${trexBonusPoints} ${t('score.units.points') || 'puntos'}</p>
            <hr style="margin: 15px 0; border: 1px solid #666;">
            <p style="font-size: 1.2em;"><strong>${t('score.label.total') || 'TOTAL'}: ${totalScore} ${t('score.units.points') || 'puntos'}</strong></p>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        try {
            ...
            fetch('/account/results', { credentials: 'same-origin' })
                .then(r => r.json())
                .then(j => {
...
async function addPlayer() {
    ...
    if (!email) {
        alert(t('newGame.alert.enterEmail') || 'Por favor, ingrese un email');
        return;
    }
    
    if (!isValidEmail(email)) {
        alert(t('newGame.alert.validEmail') || 'Por favor, ingrese un email válido');
        return;
    }
    ...
    if (gamePlayers.some(player => player.email === email)) {
        alert(t('newGame.alert.alreadyAdded') || 'Este jugador ya está añadido a la partida');
        return;
    }
    
    // Проверяем лимит на количество игроков / Verificamos el límite de jugadores
    if (gamePlayers.length >= 5) {
        alert(t('newGame.alert.maxPlayers') || 'No se pueden añadir más de 5 jugadores');
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            alert(data.message || (t('newGame.user.notFound') || 'Usuario no encontrado. El jugador debe estar registrado en el sistema.'));
        }
        
    } catch (error) {
        console.error('Error al verificar usuario:', error);
        alert(t('newGame.user.verifyError') || 'Error al verificar el usuario. Por favor, intente nuevamente.');
    }
}
...
function updatePlayersDisplay() {
    ...
    playerCount.textContent = t('newGame.playerCount', { count: gamePlayers.length }) || `Jugadores añadidos: ${gamePlayers.length}/5`;
    
    // Очищаем контейнер / Limpiamos el contenedor
    playersContainer.innerHTML = '';
    
    if (gamePlayers.length === 0) {
        const noPlayersText = t('newGame.noPlayers') || 'No hay jugadores añadidos';
        ...
        gamePlayers.forEach(player => {
            ...
            const removeLabel = t('newGame.removePlayer') || 'Eliminar';
...
function selectGameMode(selectedMode) {
    if (gamePlayers.length < 2) {
        alert(t('newGame.alert.needMinPlayers') || 'Necesitas al menos 2 jugadores para comenzar la partida');
        return;
    }
    
    if (gamePlayers.length > 5) {
        alert(t('newGame.alert.maxPlayersGame') || 'No puedes tener más de 5 jugadores');
        return;
    }
...
function showScreenWithGameManagement(screenId) {
    ...
    if (currentScreen && currentScreen.id === 'new-game' && screenId !== 'new-game') {
        const shouldReset = confirm(t('newGame.leave.confirm') || '¿Estás seguro de que quieres salir? Se perderá la configuración de la partida.');
        if (shouldReset) {
            resetNewGameData();
        } else {
